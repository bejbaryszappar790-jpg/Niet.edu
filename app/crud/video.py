...
def upload_video(db : Session, course_id : int, teacher_id : int, video_name : str, video_url : str, video_description : str, video_order_id : int):
    check_video = db.query(Video).filter(Video.course_id == course_id, Video.teacher_id == teacher_id, Video.video_url == video_url).first()

    if check_video:
        return check_video
    else:
        new_video = Video(teacher_id = teacher_id, course_id = course_id, video_name = video_name, video_url = video_url, video_order_id = video_order_id, video_description = video_description)
        db.add(new_video)
        db.commit()
